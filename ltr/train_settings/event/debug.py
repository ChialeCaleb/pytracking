...
def run(settings):
    ...
    # Validation datasets
    eotb_val = EOTB(settings.env.eotb_dir, split='val')
    ...
    proposal_params = {'boxes_per_frame': 128, 'gt_sigma': (
        0.05, 0.05), 'proposal_sigma': [(0.05, 0.05), (0.5, 0.5)]}
    label_params = {'feature_sz': settings.feature_sz,
                    'sigma_factor': output_sigma, 'kernel_sz': settings.target_filter_sz}
    label_density_params = {'feature_sz': settings.feature_sz, 'sigma_factor': output_sigma,
                            'kernel_sz': settings.target_filter_sz, 'normalize': True}

    data_processing_train = processing.EventProcessing(search_area_factor=settings.search_area_factor,
    ...
    data_processing_val = processing.EventProcessing(search_area_factor=settings.search_area_factor,
                                                     output_sz=settings.output_sz,
                                                     center_jitter_factor=settings.center_jitter_factor,
                                                     scale_jitter_factor=settings.scale_jitter_factor,
                                                     mode='sequence',
    ...
    dataset_train = sampler.EventSampler([eotb_train], [1],
    ...
    dataset_val = sampler.EventSampler([eotb_val], [1], samples_per_epoch=5000, max_gap=200,
    ...
    loss_weight = {'bb_ce': 0.0025, 'clf_ce': 0.25,
                   'clf_ce_init': 0.25, 'clf_ce_iter': 1.0}

    actor = tracking_actors.EventActor(
        net=net, objective=objective, loss_weight=loss_weight)
