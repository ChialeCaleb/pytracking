...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None):
        ...
        if max_id is None or max_id > len(visible):
            # max_id = len(visible)
            max_id = len(visible) -1 # 最后一帧图片无法作为上一帧的图片
    ...
    def __getitem__(self, index):
        ...
        train_previous_frames, train_event_stack, train_frames, train_anno, meta_obj_train,  = dataset.get_frames(seq_id, train_frame_ids, seq_info_dict)
        test_previous_frames, test_event_stack, test_frames, test_anno, meta_obj_test = dataset.get_frames(seq_id, test_frame_ids, seq_info_dict)

        # 因为 train 是模板图片，前一帧图片和事件不存在模板图片
        data = TensorDict({'train_images': train_frames,
                    'train_anno': train_anno['bbox'],
                    'train_event_stack': train_event_stack,
                    'train_previous_images': train_previous_frames,
                    'test_images': test_frames,
                    'test_anno': test_anno['bbox'],
                    'test_event_stack': test_event_stack,
                    'test_previous_images': test_previous_frames,
                    'dataset': dataset.get_name(),
                    'test_class': meta_obj_test.get('object_class_name')})

        return self.processing(data)

# 自己定义的一个 Sampler，采用因果关系采样帧
class EventSampler(TrackingSampler):
    """ See TrackingSampler."""
