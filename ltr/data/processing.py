...
def stack_tensors(x):
    ...
    elif isinstance(x, (list, tuple)) and isinstance(x[0][0], torch.Tensor):
        return [torch.stack(x[0]), torch.stack(x[1]), torch.stack(x[2])]
    return x
...
class EventProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            data['train_previous_images'], data['train_anno'] = self.transform['joint'](image=data['train_previous_images'], bbox=data['train_anno'])
            for i in range(len(data['train_event_stack'][0])):
                data['train_event_stack'][0][i], _ = self.transform['joint'](image=data['train_event_stack'][0][i], bbox=data['train_anno'])

            data['test_images'], data['test_anno'] = self.transform['joint'](image=data['test_images'], bbox=data['test_anno'], new_roll=False)
            data['test_previous_images'], data['test_anno'] = self.transform['joint'](image=data['test_previous_images'], bbox=data['test_anno'], new_roll=False)
            for i in range(len(data['test_event_stack'][0])):
                data['test_event_stack'][0][i], _ = self.transform['joint'](image=data['test_event_stack'][0][i], bbox=data['train_anno'], new_roll=False)
        ...
        for s in ['train', 'test']:
            ...
            previous_crops, previous_boxes = prutils.target_image_crop(data[s + '_previous_images'], jittered_anno, data[s + '_anno'],
                                                     self.search_area_factor, self.output_sz, mode=self.crop_type,
                                                     max_scale_change=self.max_scale_change)
            crops_event = []
            for i in range(len(data[s + '_event_stack'])):
                crops_event_tem, _ = prutils.target_image_crop([data[s + '_event_stack'][0][i],data[s + '_event_stack'][1][i],data[s + '_event_stack'][2][i]], \
                                                               jittered_anno, data[s + '_anno'], self.search_area_factor, \
